...
try {
  ...
  if (!stripeKey) {
    console.warn(
      '⚠️ STRIPE_SECRET_KEY no está configurada. Modo simulación activado.'
    )
  } else {
...
exports.procesarPago = async (req, res) => {
  try {
    ...
    if (!total || isNaN(total) || total <= 0) {
      return res
        .status(400)
        .json({ success: false, message: 'El total del pedido es inválido' })
    }
    ...
    for (const item of items) {
      if (!item.productId) {
        return res
          .status(400)
          .json({ success: false, message: 'ID de producto no proporcionado' })
      }
